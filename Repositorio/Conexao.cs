...
namespace Gerencial.Repositorio
{
    public class Conexao : IDisposable
    {
        ...
        private static string GetMySQLDbConnectionString()
        {
            return ConfigurationManager.AppSettings.Get("MYSQL_CONNECTION_STRING") ??
                   ConfigurationManager.ConnectionStrings["DBgerencial"].ConnectionString;
        }
                ...
                using (var reader = cmdComando.ExecuteReader())
                {
                    ...
                    while (reader.Read())
                    {
                        ...
                        for (var i = 0; i < reader.FieldCount; i++)
                        {
                            ...
                            var tipo = reader.GetFieldType(i).Name;
                            /*
                            if (tipo == "MySqlDateTime")
                                valorDaColuna = reader.IsDBNull(i) ? null : reader.GetValue(i);
                            else */
                            var valorDaColuna2 = reader.IsDBNull(i) ? null : reader.GetValue(i);
                            var valorDaColuna3 = reader.IsDBNull(i) ? null : reader.GetValue(i).ToString();
                            valorDaColuna = reader.IsDBNull(i) ? null : reader.GetString(i);
        ...
        public List<Dictionary<string, object>> ExecutaComandoComRetornoObjeto(string comandoSQL, Dictionary<string, object> parametros)
        {
            ...
            try
            {
                ...
                using (var reader = cmdComando.ExecuteReader())
                {
                    ...
                    while (reader.Read())
                    {
                        ...
                        {
                            var nomeDaColuna = reader.GetName(i);
                            object valorDaColuna = new object();
